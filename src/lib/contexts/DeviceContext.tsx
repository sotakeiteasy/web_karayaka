...
export const DeviceProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const isMobileUA =
    /(android|iphone|ipod|blackberry|iemobile|opera mini|windows phone|mobi|phone|smartphone|mobile|)/i.test(ua);
  const isTabletUA = /(ipad|tablet|playbook|silk|kindle|nexus 7|nexus 10|kfvow|kftt|kfot|sm-t\d+)/i.test(ua);
  const [isMobile, setIsMobile] = useState(uaDataMobile || isMobileUA || isTabletUA);
